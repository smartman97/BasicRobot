...
public class EV3Bot
{
	...
	{
		// Wheel wheel1 = DifferentialChassis.modelWheel(Motor.A,
		// 43.2).offset(-72);
		// Wheel wheel2 = DifferentialChassis.modelWheel(Motor.D,
		// 43.2).offset(72);
		// Chassis chassis = new DifferentailChassis(new Wheel[]{wheel1,
		// wheel2});
		...
		WheeledChassis chassis = new WheeledChassis(new Wheel[] { leftWheel,
				rightWheel }, WheeledChassis.TYPE_DIFFERENTIAL);
	...
	public void driveRoom()
	{
		...
		if (true) // Not a real number. Figure out a better number.
					// ultrasonicSamples[0] < .5
		{
			shortPath();
		}
		else
		...
		/*
		 * Distances 13ft by 19 ft by 12 ft by 3 ft in CM 396 by 579 by 366 by
		 * 91
		 */
		distanceSensor.fetchSample(ultrasonicSamples, 0);
	...
	private void shortPath()
	{
		...
		botPilot.travel(910);
		botPilot.rotate(65);
		botPilot.travel(3250);
		botPilot.rotate(-65);
		botPilot.travel(5100);
		botPilot.rotate(65);
		botPilot.travel(3750);
	}
	...
	{
		while (LocalEV3.get().getKeys().waitForAnyPress() != LocalEV3.get()
				.getKeys().ID_ESCAPE)
		{
